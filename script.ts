...
const normalizeData = <T extends post>(unnormalizedData: T[]): object => {
  let byId = {} as innerObject
  let allIds = []

  for (const key of unnormalizedData) {
    const { id, title, body }: post = key

    allIds.push(key.id)

    byId[key.id as keyof innerObject] = { id, title, body }
  }
